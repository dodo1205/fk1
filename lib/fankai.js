...
async function getRawPageContent(title) {
    ...
    if (json.query.pages[0].revisions) {
...
async function searchFankai(query = null, id = null) {
    ...
    try {
        ...
        while ((match = regex.exec(content)) !== null) {
            const animeDetails = await getFullAnimeDetails(match[1].trim());
            if (animeDetails) {
                animes.push(animeDetails);
            }
        }
...
async function getFullAnimeDetails(animeId) {
    try {
        ...
        const imageMatch = content.match(/image1\s*=\s*(.*)/);
