...
async function generateLinks(hash, apiKey) {
    const url = `${BASE_URL}${API_PREFIX}/generatelinks`;
    const headers = { 
        ...getHeaders(apiKey),
        'Content-Type': 'application/json' 
    };
    ...
    if (!response.data.success || !response.data.data) {
        console.warn('[TorboxAPI] Réponse inattendue de generateLinks:', response.data);
        throw new Error('Torbox: Réponse inattendue de generateLinks');
    }
    // La réponse est { success: true, data: { "hash1": [...] } }
    return response.data.data;
}
