...
async function getTorrentCachedAvailability(hashes, apiKey) {
    const url = `${BASE_URL}/api/torrents/cached`;
    const params = new URLSearchParams();
    hashes.forEach(hash => params.append('hash', hash));

    const response = await axios.get(url, { params, headers: getHeaders(apiKey) });
    if (!response.data.success) {
        throw new Error('Failed to get torrent cached availability from Torbox');
    }
    return response.data.data;
}
