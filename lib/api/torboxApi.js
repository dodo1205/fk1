...
async function getTorrentInfoById(torrentId, apiKey) {
    try {
        // D'abord essayer via la liste avec bypass_cache (plus fiable et plus frais)
        ...
        // Fallback 1: essayer sans bypass_cache
        ...
        // Fallback 2: endpoint direct
        const directUrl = `${BASE_URL}${API_PREFIX}/torrentinfo`;
        const params = { torrent_id: torrentId };
        const directResponse = await axios.get(directUrl, { params, headers: getHeaders(apiKey) });
        if (!directResponse.data.success) {
            throw new Error(`Torbox: Failed to get torrent info for ID ${torrentId}. Message: ${directResponse.data.message}`);
        }
