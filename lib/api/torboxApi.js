...
async function getTorrentInfo(identifier, apiKey, isHash = false) {
    const url = `${BASE_URL}${API_PREFIX}/torrentinfo`; 
    const params = isHash ? { hash: identifier } : { torrent_id: identifier };
    const response = await axios.get(url, { params, headers: getHeaders(apiKey) });
    if (!response.data.success) {
        throw new Error(`Torbox: Failed to get torrent info for ${identifier}. Message: ${response.data.message}`);
    }
    return response.data.data; 
}
