...
function selectBestFile(files, episode, episodeName = null, options = {}) {
    ...
    const { fileIndex, service = 'unknown', torrentFilename } = options;
    console.log(`[${service.toUpperCase()}] Sélection de fichier pour l'épisode ${episode}${episodeName ? ', nom: ' + episodeName : ''}`);
    if (torrentFilename) {
        console.log(`[${service.toUpperCase()}] Contexte du nom de torrent: ${torrentFilename}`);
    }
    ...
    // On passe maintenant le nom du torrent en plus pour une meilleure correspondance
    const bestFile = findBestEpisodeFile(videoFiles, episode, episodeName, torrentFilename);
    if (bestFile) {
        console.log(`[${service.toUpperCase()}] Fichier vidéo sélectionné pour l'épisode ${episode}: ${bestFile.name}`);
        return bestFile;
    }
