...
const { encodeBase64UrlSafe } = require('./utils/stringUtils');
...
function formatStatus(status) {
    if (!status) return 'Fan-Kai';
    const lowerStatus = status.toLowerCase();
    if (lowerStatus === 'finis') return 'Terminé';
    if (lowerStatus.startsWith('a jour') || lowerStatus.startsWith('à jour')) return 'À jour';
    return status;
}
...
builder.defineCatalogHandler(async ({ type, id, extra }) => {
    ...
    try {
        ...
        if (id === 'fankai-tout') {
            ...
            filteredAnimes = visibleAnimes.filter(anime => anime.status && (anime.status.toLowerCase().startsWith('à jour') || anime.status.toLowerCase().startsWith('a jour')));
        } else {
        ...
        const metas = filteredAnimes.map(anime => {
            ...
            return {
                ...
                releaseInfo: formatStatus(anime.status),
...
builder.defineMetaHandler(async ({ type, id }) => {
    ...
    try {
        ...
        // Pré-remplir la map des saisons
        if (anime.seasons && anime.seasons.length > 0) {
            anime.seasons.forEach(season => {
                seasonsMap.set(String(season.id), season);
            });
        }
...
builder.defineStreamHandler(async ({ type, id }) => {
    ...
    try {
        ...
        for (let i = 0; i < torrentsToProcess.length; i++) {
            ...
            description += `🔍 Nyaa 💾 ${sizeText} 👥 ${seeders}`;

            console.log(`[STREAM #${i + 1}] Ajout de ${rawTitle} (${seeders} seeders)`);

            // --- New Stream Options Logic ---
            if (debridConfig.service === 'none' || !debridConfig.apiKey) {
                ...
                // Debrid service is selected. Create a single stream entry that will be resolved by the backend.
                const numericEpisodeNumber = parseInt(episodeNumber);
                const episodeName = details.episodeName || null;
                const seasonNumber = details.seasonNumber || null; // Assuming getTorrentDetails can provide this

                // The queryData will be passed to the /playback endpoint
                // It now includes the 'service' to be used, determined by the user's addon configuration.
                const queryData = {
                    magnet: magnetLink,
                    service: debridConfig.service, // e.g., 'alldebrid', 'realdebrid'
                    episode: numericEpisodeNumber,
                    episodeName: episodeName,
                    season: seasonNumber, // Pass season if available
                    type: isAnimeMovie(details) ? 'movie' : 'series', // Pass stream type
                    // fileIndex might be needed for some debrids if multiple files match,
                    // but selectBestFile in BaseDebridService should handle most cases.
                    // We can add fileIndex later if specific debrid services require it pre-selection.
                };

                const queryB64 = encodeBase64UrlSafe(JSON.stringify(queryData));
                const playbackUrl = `${debridConfig.baseUrl}/playback/${queryB64}`;
                
                const serviceNameFormatted = debridConfig.service.charAt(0).toUpperCase() + debridConfig.service.slice(1);

                // We no longer explicitly check cache here. The backend (resolveStream) will handle it.
                // The name can reflect the service, or be more generic like "Play with [Service]"
                // For simplicity, let's use a name that indicates it will try to play or cache.
                streams.push({
                    name: `▶️ Jouer via\n${serviceNameFormatted}`,
                    description: description, // Keep detailed description
                    url: playbackUrl,
                    behaviorHints: {
                        bingeGroup: bingeGroup,
                        filename: filename
                        // We assume it's webReady, or the backend will provide a "caching" video.
                    }
                });
            }
        }

        // Trier les streams: Debrid links first, then by seeders. Magnet links last.
        streams.sort((a, b) => {
            const isDebridA = !a.url.startsWith('magnet:');
            const isDebridB = !b.url.startsWith('magnet:');

            if (isDebridA && !isDebridB) return -1; // Debrid A first
            if (!isDebridA && isDebridB) return 1;  // Debrid B first

            // If both are debrid or both are magnet, sort by seeders (extracted from description)
