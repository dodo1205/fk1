...
const { encodeBase64UrlSafe } = require('./stringUtils');
...
const manifest = {
    ...
    background: '', 
    catalogs: [
        { id: 'fankai-tout', name: 'Tout', type: 'fankai', extra: [ { name: 'search', isRequired: false }, { name: 'genre' } ] },
        { id: 'fankai-terminees', name: 'Terminées', type: 'fankai', extra: [ { name: 'search', isRequired: false }, { name: 'genre' } ] },
        { id: 'fankai-encours', name: 'En cours', type: 'fankai', extra: [ { name: 'search', isRequired: false }, { name: 'genre' } ] },
        { id: 'fankai-ajour', name: 'À jour', type: 'fankai', extra: [ { name: 'search', isRequired: false }, { name: 'genre' } ] }
    ],
...
builder.defineCatalogHandler(async ({ type, id, extra }) => {
    ...
    try {
        ...
        let catalogFilteredAnimes = visibleAnimes;
        // Gestion des catalogues, avec un fallback si l'ID n'est pas reconnu
        if (id === 'fankai-tout') {
            catalogFilteredAnimes = visibleAnimes;
        } else if (id === 'fankai-terminees') {
            catalogFilteredAnimes = visibleAnimes.filter(anime => anime.status === 'Finis');
        } else if (id === 'fankai-encours') {
            catalogFilteredAnimes = visibleAnimes.filter(anime => anime.status === 'En cours');
        } else if (id === 'fankai-ajour') {
            // Correction pour gérer les statuts corrompus comme "A' jour"
            catalogFilteredAnimes = visibleAnimes.filter(anime => anime.status && (anime.status.toLowerCase().startsWith('à jour') || anime.status.toLowerCase().startsWith('a jour')));
        } else {
            console.log(`Catalogue inconnu ${id}, utilisation du catalogue par défaut 'fankai-tout'`);
            catalogFilteredAnimes = visibleAnimes; // Fallback vers 'Tout' pour les catalogues inconnus comme 'top'
        }

        // Appliquer le filtre de genre si présent
        let filteredAnimes = catalogFilteredAnimes;
        if (extra.genre) {
            console.log(`Filtrage par genre: ${extra.genre}`);
            filteredAnimes = catalogFilteredAnimes.filter(anime => 
                anime.genres && anime.genres.map(g => g.toLowerCase()).includes(extra.genre.toLowerCase())
            );
        }
    ...
    try {
        ...
        for (let i = 0; i < torrentsToProcess.length; i++) {
            ...
            if (debridConfig.service === 'none' || !debridConfig.apiKey) {
                ...
                const cacheConfig = { ...debridConfig, downloadOption: 'cached' };
                const cacheResult = await debridTorrent(magnetLink, cacheConfig, numericEpisodeNumber, episodeName);
                const isCached = cacheResult && cacheResult.status === 'completed';
