...
// On importe les deux fonctions maintenant
const { checkCache, debridTorrent } = require('./debrid/index'); 
    ...
    try {
        ...
        for (let i = 0; i < torrentsToProcess.length; i++) {
            ...
            if (debridConfig.service === 'none' || !debridConfig.apiKey) {
                ...
                // --- NOUVELLE LOGIQUE DE PHASE 1 ---
                console.log(`[PHASE 1] Vérification du cache pour torrent ${i+1}`);
                const cacheResult = await checkCache(magnetLink, debridConfig);
                const isCached = cacheResult ? cacheResult.isCached : false;

                console.log(`[PHASE 1] Résultat du cache pour torrent ${i+1}: ${isCached ? 'EN CACHE' : 'NON EN CACHE'}`);

                const baseQuery = {
                    magnet: magnetLink,
                    episode: numericEpisodeNumber,
                    episodeName: episodeName,
                    filename: torrent.title // Ajout crucial du nom de fichier du torrent
                };
                ...
                if (isCached) {
                    ...
                    // Pour Torbox, on ajoute une information cruciale à l'URL pour la Phase 2
                    const isTorboxCached = debridConfig.service === 'torbox';
                    const playUrl = `${debridConfig.baseUrl}/playback/${playQueryB64}${isTorboxCached ? '?cached=true' : ''}`;
                    streams.push({
