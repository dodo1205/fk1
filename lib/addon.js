...
builder.defineStreamHandler(async ({ type, id }) => {
    ...
    try {
        ...
        if (debridConfig.service !== 'none' && debridConfig.apiKey) {
            const hashes = torrentsToProcess.map(torrent => {
                const magnetLink = torrent.magnetLink || torrent.magnetUrl;
                const infoHashMatch = magnetLink.match(/urn:btih:([a-zA-Z0-9]+)/i);
                return infoHashMatch ? infoHashMatch[1].toLowerCase() : null;
            }).filter(hash => hash);

            const { checkTorrentsCached } = require('./debrid/index');
            const cachedStatuses = await checkTorrentsCached(hashes, debridConfig);
            ...
            for (let i = 0; i < torrentsToProcess.length; i++) {
                ...
                const infoHashMatch = magnetLink.match(/urn:btih:([a-zA-Z0-9]+)/i);
                const infoHash = infoHashMatch ? infoHashMatch[1].toLowerCase() : null;
                const isCached = cachedStatuses[infoHash];
                ...
                if (isCached) {
                    ...
                    streams.push({
                        ...
                        description: `[CACHE] ${torrent.title}`,
                        url: playUrl,
                        behaviorHints: {
                            bingeGroup: `fankai-${infoHash || Math.random()}`,
                    ...
                    streams.push({
                        name: `⬇️ Télécharger\n${serviceNameFormatted}`,
                        description: `[DOWNLOAD] ${torrent.title}`,
                        url: downloadUrl,
                        behaviorHints: {
                            bingeGroup: `fankai-${infoHash || Math.random()}`,
                            filename: torrent.title
