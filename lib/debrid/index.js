...
async function debridTorrent(magnetLink, config, episodeNumber, episodeName, options = {}) {
    ...
    try {
        ...
        if (normalizedServiceName === 'realdebrid' || normalizedServiceName === 'alldebrid') {
            ...
            const cacheInfo = await checkCache(magnetLink, config);
            const debridOptions = { ...options, isCached: cacheInfo ? cacheInfo.isCached : false };
            streamResult = await service.getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, debridOptions);
        } else {
