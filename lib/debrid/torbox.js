        ...
        try {
            ...
            while (Date.now() - startTime < pollingTimeout) {
                ...
                if (currentInfo) {
                    ...
                    // Le statut peut être 'completed', 'cached', ou 'download ready'
                    const normalizedStatus = (status || '').toLowerCase();
                    if (files && files.length > 0 && (normalizedStatus === 'completed' || normalizedStatus === 'cached' || normalizedStatus === 'download ready')) {
