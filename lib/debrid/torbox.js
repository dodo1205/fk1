...
class Torbox extends DebridService {
    ...
    async getTorrentInfo(torrentId) {
        try {
            ...
            // Gérer spécifiquement l'erreur 422 comme un état temporaire
            if (error.response && error.response.status === 422) {
                ...
                return { status: 'processing' }; // Retourner un statut personnalisé pour que la boucle continue
            }
    ...
    async getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, options = {}) {
        ...
        try {
            ...
            if (isCached) {
                console.log('[Torbox] Le lien est marqué comme EN CACHE. Tentative de récupération directe des fichiers via checkCache.');
