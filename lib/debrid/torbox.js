...
class Torbox extends DebridService {
    ...
    async getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, options = {}) {
        ...
        try {
            ...
            const existingTorrent = myTorrents.find(t => {
                const torrentHash = t.hash.toLowerCase();
                const searchHash = hash.toLowerCase();
                // --- DEBOGAGE FINAL ---
                console.log(`[Torbox DEBUG] Comparaison: "${torrentHash}" (depuis API) vs "${searchHash}" (recherché). Sont-ils égaux ? ${torrentHash === searchHash}`);
                return torrentHash === searchHash;
            });
