...
class Torbox extends DebridService {
    ...
    async getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, options = {}) {
        ...
        try {
            ...
            // Attendre que le torrent soit prêt pour le téléchargement (avec tolérance pour Torbox)
            console.log(`[Torbox] Attente du téléchargement du torrent ${torrentId}...`);
            const startTime = Date.now();
            let consecutiveErrors = 0;
            const maxConsecutiveErrors = 10; // Plus tolérant
            ...
            while (Date.now() - startTime < pollingTimeout) {
                try {
                    ...
                    if (torrentInfo) {
                        ...
                        // Vérifier si le torrent est prêt
                        ...
                        consecutiveErrors++;
                        console.warn(`[Torbox] Torrent ${torrentId} temporairement non trouvé (${consecutiveErrors}/${maxConsecutiveErrors})`);
                    }
                } catch (error) {
                    consecutiveErrors++;
                    console.warn(`[Torbox] Erreur temporaire ${consecutiveErrors}/${maxConsecutiveErrors}: ${error.message}`);
                    
                    if (consecutiveErrors >= maxConsecutiveErrors) {
                        console.error(`[Torbox] Trop d'erreurs consécutives, abandon pour ${torrentId}`);
                        return null;
                    }
                }
                
                await new Promise(resolve => setTimeout(resolve, 15000)); // Attente plus longue (15s)
            }
