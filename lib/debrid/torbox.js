...
class Torbox extends DebridService {
    ...
    async getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, options = {}) {
        ...
        try {
            // NOUVELLE LOGIQUE POUR TORBOX
            if (isCached) {
                console.log('[Torbox] Le lien est marqué comme EN CACHE. Tentative de récupération directe des fichiers.');
                // Si le lien est en cache, on n'a pas besoin de l'ajouter ni de faire du polling.
                // On a juste besoin de la liste des fichiers pour sélectionner le bon.
                const cacheInfo = await this.checkCache(magnetLink);
                if (cacheInfo && cacheInfo.isCached && cacheInfo.files && cacheInfo.files.length > 0) {
                    const filesForSelection = cacheInfo.files.map(f => ({ name: f.name, path: f.name, size: f.size, url: f.url, ...f }));
                    ...
                    if (bestFile && bestFile.url) {
                        console.log(`[Torbox] Fichier trouvé dans le cache et lien direct obtenu: ${bestFile.url}`);
                        return { streamUrl: bestFile.url, filename: bestFile.name };
                    }
                }
                console.warn('[Torbox] Le lien était marqué EN CACHE, mais la récupération des fichiers a échoué. Passage à la méthode standard.');
                // Si la récupération directe échoue, on continue avec la méthode normale ci-dessous.
            }

            // LOGIQUE STANDARD (si non-caché ou si la récupération directe du cache a échoué)
