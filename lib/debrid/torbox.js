    ...
    async getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, options = {}) {
        ...
        try {
            if (isCached) {
                ...
                const torrentInfo = await this.api.getTorrentInfoByHash(hash, this.apiKey);
                if (!torrentInfo || !torrentInfo.files || torrentInfo.files.length === 0) {
                    throw new Error('Le torrent est en cache mais getTorrentInfoByHash n\'a retourné aucun fichier.');
                }
                if (!torrentInfo.id) {
                    throw new Error('Le torrent est en cache mais getTorrentInfoByHash n\'a pas retourné de torrent_id.');
                }
            ...
            while (Date.now() - startTime < pollingTimeout) {
                ...
                const status = (torrentInfo.download_state || '').toLowerCase();
