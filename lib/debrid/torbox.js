...
class Torbox extends DebridService {
    ...
    async checkApiKey() {
        try {
            // Correct method call, using the plural 'torrents' service
    ...
    async findExistingTorrent(infoHash) {
        const torrentsResult = await this.client.torrents.list();
    ...
    async addMagnetOnly(magnetLink) {
        try {
            ...
            const result = await this.client.torrents.create({ magnet: magnetLink });
    ...
    async getTorrentStatusAndLinks(magnetLink, fileIndex, season, episode, streamType, episodeName) {
        try {
            ...
            const torrentInfoResult = await this.client.torrents.get({ torrent_id: existingTorrent.id });
            ...
            const downloadLinkResult = await this.client.torrents.requestDownload({ torrent_id: existingTorrent.id, file_id: bestFile.id });
