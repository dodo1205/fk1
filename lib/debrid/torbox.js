...
class Torbox extends DebridService {
    ...
    async checkApiKey() {
        try {
            // Corrected method call based on service naming conventions
            await this.client.torrentsService.list();
            return true;
        } catch (error) {
    ...
    async findExistingTorrent(infoHash) {
        const torrentsResult = await this.client.torrentsService.list();
    ...
    async addMagnetOnly(magnetLink) {
        try {
            ...
            const result = await this.client.torrentsService.create({ magnet: magnetLink });
    ...
    async getTorrentStatusAndLinks(magnetLink, fileIndex, season, episode, streamType, episodeName) {
        try {
            ...
            const torrentInfoResult = await this.client.torrentsService.get({ torrent_id: existingTorrent.id });
            ...
            const downloadLinkResult = await this.client.torrentsService.requestDownload({ torrent_id: existingTorrent.id, file_id: bestFile.id });
