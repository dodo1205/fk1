    ...
    async getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, options = {}) {
        ...
        try {
            ...
            if (existingTorrent) {
                ...
                // Vérifier immédiatement le statut du torrent existant
                ...
                // Si déjà complété, pas besoin d'attendre
                if (currentStatus === 'completed' || currentStatus === 'download ready' || currentStatus === 'cached') {
                    console.log(`[Torbox] Torrent déjà complété, passage direct à la sélection de fichier`);
                    
                    // Récupérer les infos complètes du torrent
            ...
            while (Date.now() - startTime < pollingTimeout) {
                ...
                if (torrentInfo) {
                    ...
                    console.log(`[Torbox] Détails torrent:`, {
                        status: torrentInfo.status,
                        progress: torrentInfo.progress,
                        filesCount: torrentInfo.files ? torrentInfo.files.length : 0
                    });

                    if (status === 'completed' || status === 'download ready' || status === 'cached') {
                        if (torrentInfo.files && torrentInfo.files.length > 0) {
                            console.log(`[Torbox] Torrent ${torrentId} complété avec ${torrentInfo.files.length} fichier(s).`);
                            console.log(`[Torbox] Fichiers disponibles:`, torrentInfo.files.map(f => ({
                                id: f.id,
                                name: f.name,
                                size: f.size
                            })));
                    ...
                    console.log(`[Torbox] Attente... ID: ${torrentId}, impossible de récupérer les infos.`);
                }
