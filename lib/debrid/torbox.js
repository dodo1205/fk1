...
class Torbox extends DebridService {
    ...
    async getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, options = {}) {
        const { pollingTimeout = 300000, pollingInterval = 5000, filename: torrentFilename, isCached } = options;
        let torrentId;
        ...
        try {
            ...
            if (existingTorrent) {
                ...
                // Petit délai pour laisser l'API traiter la nouvelle entrée
                await new Promise(resolve => setTimeout(resolve, 2000));
            }

            // Étape 2: Vérifier si on doit attendre le téléchargement (uniquement si non-caché)
            if (!isCached) {
                console.log(`[Torbox] Torrent non-caché. Lancement du polling pour ID: ${torrentId}.`);
                const startTime = Date.now();
                while (Date.now() - startTime < pollingTimeout) {
                    const torrentInfo = await this.getTorrentInfo(torrentId);
                    if (torrentInfo && (torrentInfo.status === 'completed' || torrentInfo.progress === 100)) {
                        console.log(`[Torbox] Téléchargement complété pour le torrent ${torrentId}.`);
                        break; // Sortir de la boucle, on est prêt
                    }
                    if (!torrentInfo || torrentInfo.status === 'error' || torrentInfo.status === 'stalled') {
                        throw new Error(`Erreur avec le torrent ${torrentId}. Statut: ${torrentInfo ? torrentInfo.status : 'inconnu'}`);
                    }
                    console.log(`[Torbox] Statut du torrent ${torrentId}: ${torrentInfo.status} (Progression: ${torrentInfo.progress || 0}%)`);
                ...
                console.log(`[Torbox] Torrent marqué comme étant en cache. On saute la boucle d'attente.`);
            }

            // Étape 3: Obtenir les infos, sélectionner le fichier et récupérer le lien
            const finalTorrentInfo = await this.getTorrentInfo(torrentId);
            if (!finalTorrentInfo || !finalTorrentInfo.files || finalTorrentInfo.files.length === 0) {
                throw new Error(`Impossible de récupérer les infos finales ou aucun fichier trouvé pour le torrent ${torrentId}.`);
            }
