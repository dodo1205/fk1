...
class Torbox extends DebridService {
    ...
    async checkCache(magnetLink) {
        ...
        try {
            ...
            if (response.success && response.data) {
                console.log(`[Torbox] Cache check pour hash ${hash}: cached=${response.data.cached}`);
                return {
                    isCached: response.data.cached || false,
                    files: response.data.files || [], // files est présent si list_files=true et cached=true
                    torrentIdIfPresent: response.data.torrent_id || null // Si le torrent est déjà dans la liste de l'utilisateur
                };
            }
    ...
    async addMagnet(magnetLink, name = null) {
        try {
            ...
            // Utiliser createTorrentAsync pour ne pas bloquer.
            const response = await this.api.createTorrentAsync(magnetLink, this.apiKey, name);
            if (response && response.torrent_id) {
                console.log(`[Torbox] Magnet ajouté (async) avec succès. ID: ${response.torrent_id}`);
                return response.torrent_id;
            }
            console.error('[Torbox] Réponse inattendue de createTorrentAsync:', response);
            return null;
        } catch (error) {
