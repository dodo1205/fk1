...
class Torbox extends DebridService {
    ...
    async getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, options = {}) {
        ...
        try {
            // ** NOUVELLE LOGIQUE **
            if (isCached) {
                console.log('[Torbox] Le lien est marqué comme EN CACHE. Génération des liens directs...');
                const linksData = await this.api.generateLinks(hash, this.apiKey);
                const files = linksData[hash];
                if (!files || files.length === 0) {
                    throw new Error('Le torrent est en cache mais generateLinks n\'a retourné aucun fichier.');
                }

                const filesForSelection = files.map(f => ({
                    name: f.name,
                    path: f.name, 
                    size: f.size,
                    url: f.url,
                ...
                console.log(`[Torbox] Lien de streaming direct obtenu du cache: ${bestFile.url}`);
                return { streamUrl: bestFile.url, filename: bestFile.name };
            }

            // ** LOGIQUE STANDARD POUR LES TORRENTS NON-CACHÉS **
            ...
            while (Date.now() - startTime < pollingTimeout) {
                ...
                if (status === 'completed' || torrentInfo.progress === 100) {
                    if (torrentInfo.files && torrentInfo.files.length > 0) {
                        ...
                        console.log(`[Torbox] Lien de streaming obtenu: ${bestFile.url}`);
                        return { streamUrl: bestFile.url, filename: bestFile.name };
                    }
