...
class Torbox extends DebridService {
    ...
    async getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, options = {}) {
        ...
        try {
            ...
            // Attendre que les métadonnées du torrent soient disponibles (comme stream-fusion)
            console.log(`[Torbox] Attente des métadonnées du torrent ${torrentId}...`);
            ...
            while (Date.now() - startWaitTime < metadataTimeout) {
                ...
                if (torrentInfo && torrentInfo.files && torrentInfo.files.length > 0) {
                    console.log(`[Torbox] Métadonnées récupérées pour ${torrentId}. ${torrentInfo.files.length} fichier(s) trouvé(s).`);
                    break;
                }
                console.log(`[Torbox] Pas de fichiers encore disponibles, attente 3s...`);
