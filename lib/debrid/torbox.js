...
class Torbox extends DebridService {
    ...
    async getTorrentInfo(identifier, isHash = false, retries = 3) {
        try {
            return await this.api.getTorrentInfo(identifier, this.apiKey, isHash);
        } catch (error) {
            if (error.response && error.response.status === 422 && retries > 0) {
                console.warn(`[Torbox] Erreur 422 pour getTorrentInfo sur ${identifier}. Nouvelle tentative dans 2s... (${retries} restantes)`);
                await new Promise(resolve => setTimeout(resolve, 2000));
                return this.getTorrentInfo(identifier, isHash, retries - 1);
            }
            if (error.response && error.response.status === 422) {
                console.warn(`[Torbox] Erreur 422 persistante pour getTorrentInfo sur ${identifier}. On continue le polling.`);
