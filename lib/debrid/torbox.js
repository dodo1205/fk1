...
class Torbox extends DebridService {
    ...
    async getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, options = {}) {
        ...
        try {
            ...
            // CORRECTION: Récupérer une liste fraîche à chaque fois
            ...
            if (existingTorrent) {
                ...
                // CORRECTION: Vérifier avec une liste fraîche après l'ajout
                let torrentFound = false;
                for (let attempt = 0; attempt < 5; attempt++) {
                    const waitTime = 3000 + (attempt * 1000); // 3s, 4s, 5s, 6s, 7s
                    console.log(`[Torbox] Attente de ${waitTime}ms avant vérification ${attempt + 1}/5...`);
                    await new Promise(resolve => setTimeout(resolve, waitTime));
                    
                    try {
                        // IMPORTANT: Récupérer une nouvelle liste fraîche
                        myTorrents = await this.api.getMyTorrents(this.apiKey);
                        const foundTorrent = myTorrents.find(t => t.id == torrentId);
                        
                        if (foundTorrent) {
                            console.log(`[Torbox] ✅ Torrent ${torrentId} confirmé dans la liste après ${waitTime}ms (tentative ${attempt + 1})`);
                            console.log(`[Torbox] Statut du torrent trouvé: ${foundTorrent.status}, Progression: ${foundTorrent.progress || 0}%`);
                            torrentFound = true;
                            existingTorrent = foundTorrent; // Utiliser le torrent trouvé
                            break;
                        } else {
                            console.log(`[Torbox] ❌ Torrent ${torrentId} non trouvé dans la liste (tentative ${attempt + 1}), ${myTorrents.length} torrents total`);
                        }
                    } catch (error) {
                ...
                if (!torrentFound) {
                    console.error(`[Torbox] ÉCHEC: Torrent ${torrentId} non trouvé après 5 tentatives`);
                    console.log(`[Torbox] Dernière liste récupérée contient ${myTorrents.length} torrents`);
                    // Afficher les IDs des derniers torrents pour debug
                    const lastTorrentIds = myTorrents.slice(-5).map(t => `ID:${t.id}`).join(', ');
                    console.log(`[Torbox] Derniers torrents dans la liste: ${lastTorrentIds}`);
                    throw new Error(`Torrent ${torrentId} ajouté mais non trouvé dans la liste après vérifications`);
                }
                
                // Si le torrent est déjà complété après l'ajout
                const status = (existingTorrent.status || '').toLowerCase();
                if (status === 'completed' || status === 'download ready' || status === 'cached') {
                    console.log(`[Torbox] Torrent ajouté et déjà complété, traitement direct`);
                    return await this.processCompletedTorrent(existingTorrent, episodeNumber, episodeName, { fileIndex, streamType, torrentFilename });
                }
            } else {
