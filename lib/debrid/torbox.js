...
class Torbox extends DebridService {
    constructor(apiKey) {
        ...
        this.apiVersion = 'v1'; // Or whatever version is appropriate
    }

    async checkApiKey() {
        try {
            await this.client.torrents.getTorrentList(this.apiVersion);
            return true;
        } catch (error) {
    ...
    async findExistingTorrent(infoHash) {
        const torrentsResult = await this.client.torrents.getTorrentList(this.apiVersion);
    ...
    async addMagnetOnly(magnetLink) {
        try {
            ...
            const result = await this.client.torrents.createTorrent(this.apiVersion, { magnet: magnetLink });
    ...
    async getTorrentStatusAndLinks(magnetLink, fileIndex, season, episode, streamType, episodeName) {
        try {
            ...
            // The torrent list gives us the status, no need for a separate getInfo call
            const torrentInfo = existingTorrent;
            ...
            const downloadLinkResult = await this.client.torrents.requestDownloadLink(this.apiVersion, { torrent_id: existingTorrent.id, file_id: bestFile.id });
