...
class Torbox extends DebridService {
    ...
    async getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, options = {}) {
        ...
        try {
            ...
            // --- DEBOGAGE ---
            console.log(`[Torbox DEBUG] ${myTorrents.length} torrent(s) trouvé(s) dans la liste de l'utilisateur.`);
            const allHashes = myTorrents.map(t => t.hash.toLowerCase());
            console.log(`[Torbox DEBUG] Hash recherché: ${hash}`);
            console.log(`[Torbox DEBUG] Hashes trouvés: ${JSON.stringify(allHashes)}`);
            // --- FIN DEBOGAGE ---
