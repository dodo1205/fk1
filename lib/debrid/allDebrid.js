...
class AllDebrid extends DebridService {
    ...
    async getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, options = {}) {
        const { pollingTimeout = 300000, pollingInterval = 7000 } = options; // AllDebrid peut être plus lent à lister les fichiers
        let magnetDetails;
        ...
        try {
            ...
            while (Date.now() - startTime < pollingTimeout) {
                ...
                if (magnetDetails.status === 'READY' || magnetDetails.status === 'finished') {
                    if (magnetDetails.links && magnetDetails.links.length > 0) {
                        console.log(`[AllDebrid] Magnet ${magnetId} prêt avec ${magnetDetails.links.length} fichier(s)/lien(s).`);
                        
                        // Étape 3: Sélectionner le meilleur fichier parmi magnetDetails.links
                        // Structure d'un lien: { filename: string, link: string (lien interne AD), size: number }
                        ...
                        const bestFile = this.selectBestFile(filesForSelection, episodeNumber, episodeName, { fileIndex, streamType, service: 'alldebrid' });
                        ...
                        // Étape 4: Dérestreindre le lien du meilleur fichier
                        ...
                        console.warn(`[AllDebrid] Magnet ${magnetId} marqué comme READY/finished mais aucun lien trouvé. Attente...`);
                        // Parfois, il y a un délai entre le statut READY et la disponibilité des liens.
                    }
                } else if (['error', 'too_many', 'MAGNET_FILE_UPLOAD_FAILED', 'MAGNET_INVALID_URI'].includes(magnetDetails.status) || magnetDetails.error) {
