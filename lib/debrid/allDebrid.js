    ...
    async getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, options = {}) {
        ...
        try {
            ...
            while (Date.now() - startTime < pollingTimeout) {
                ...
                // La condition de succès est que le statut soit 'READY' ou 'finished' ET que la liste des liens soit disponible et non vide.
                if ((magnetDetails.status === 'READY' || magnetDetails.status === 'finished') && magnetDetails.links && magnetDetails.links.length > 0) {
