...
class AllDebrid extends DebridService {
    ...
    async checkApiKey() {
        try {
            // La lib peut avoir une méthode user.get() ou similaire.
            // Souvent, un appel simple comme récupérer les liens de l'utilisateur suffit.
            await this.AD.user.get(); // Ou une autre méthode de test de la clé
            return true;
        } catch (error) {
            console.error('[FK AllDebrid] API key check failed:', error.message);
            return false;
        }
    }
