...
class RealDebrid extends DebridService {
    ...
    async getStreamableLinkForMagnet(magnetLink, fileIndex, season, episodeNumber, streamType, episodeName, options = {}) {
        ...
        try {
            ...
            const bestFile = this.selectBestFile(filesForSelection, episodeNumber, episodeName, { fileIndex, streamType, service: 'realdebrid', torrentFilename });
            ...
            while (Date.now() - startTime < pollingTimeout) {
                ...
                if (status === 'downloaded') {
                    ...
                    const selectedFilePath = bestFile.path;
                    let internalLink = torrentInfo.links.find(link => {
                        try {
                    ...
                    if (!internalLink && torrentInfo.links.length > 0) {
                        console.warn(`[RealDebrid] Correspondance de lien non trouvée, utilisation du premier lien par défaut: ${torrentInfo.links[0]}`);
                        internalLink = torrentInfo.links[0];
                    }

                    if (!internalLink) {
                        console.error(`[RealDebrid] Aucun lien interne trouvé pour le fichier sélectionné après téléchargement.`);
                        return null;
                    }

                    console.log(`[RealDebrid] Lien interne trouvé: ${internalLink}. Tentative de dérestriction...`);

                    // Étape 6: Toujours dérestreindre le lien pour obtenir le lien de streaming final.
                    const unrestrictedResult = await this.unrestrictLink(internalLink);
                    if (!unrestrictedResult || !unrestrictedResult.download) {
                        console.error(`[RealDebrid] Échec de la dérestriction du lien: ${internalLink}`);
                        return null;
                    }

                    const finalStreamUrl = unrestrictedResult.download;
                    console.log(`[RealDebrid] Lien de streaming final obtenu pour ${bestFile.name}: ${finalStreamUrl}`);
