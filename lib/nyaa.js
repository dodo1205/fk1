...
async function searchNyaaByUser(query) {
    try {
        const url = `https://nyaa.si/?page=rss&u=Fan-Kai&q=${encodeURIComponent(query)}`;
...
async function findRelevantTorrents(torrents, episodeNumber, episodeName) {
    ...
    for (const torrent of torrents) {
        ...
        const fileList = await getFileListFromTorrent(torrent.downloadLink);
...
async function getFileListFromTorrent(torrentUrl) {
    try {
        const response = await axios.get(torrentUrl, { responseType: 'arraybuffer' });
        const torrentData = parseTorrent(response.data);
        return torrentData.files.map(file => ({ name: file.path, size: file.length }));
    } catch (error) {
        console.error(`Erreur lors de la récupération de la liste des fichiers du .torrent: ${error.message}`);
        return [];
    }
}
